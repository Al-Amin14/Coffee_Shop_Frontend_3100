        ...
        <div className="w-[60%] mx-auto mt-12 p-6">
            <div className="bg-gradient-to-br from-[#d7ccc8] to-[#a1887f] shadow-xl rounded-2xl p-8 text-center text-white mb-10">
                <div className="flex flex-col items-center mb-6">
                    <div classNamem="w-24 h-24 rounded-full bg-gradient-to-r from-[#6d4c41] to-[#3e2723] flex items-center justify-center text-white text-3xl font-bold shadow-lg">
                ...
                {JSON.parse(localStorage.getItem('user')).role ==='Manager' && ( loading ? (
                    ...
                    <p className="mt-3 text-[#4e342e]fornt-medium">Brewing your orders..</p>
                    </div>
                ) : orders.length === 0 ? (
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        {orders.map((order) => (
                            <div key={order.id} className="p-5 rounded_xl shadow-md bg-[#efebe9]border border-[#d7ccc8]">
                                <h4 className="text-lg font-semibold text-[#4e342e] text-lg">{order.product_name}</h4>
                                <p className="text-sm text-gray-700">
                                Quantity: <span className="font-medium">{order.quantity}</span>
                            </p>
                                <p className="text-sm text-gray-700">
                                    status: <span className="font-medium">{order.status}</span></p>
                                    <p className="text-sm text-gray-500">
                                        Ordered on: {new Date(order.created_at).toLocaleDateString("en-GB")}</p>
                                        </div>
                        ))}
                    </div>
       ) )}
